...
`;
    ...
    createBot(username,port);
        j = j+1;
         app.get(`/${username}/stop`, (req, res) => {
    fetch(`http://localhost:${port}/stop`)
      .then(response => response.text())
      .then(position => {
        res.send(`Current Position: ${position}`);
      });
  });

  app.get(`/${username}/sendChat`, (req, res) => {
    const message = req.query.message;
    fetch(`http://localhost:${port}/chat?message=${encodeURIComponent(message)}`);
    res.send(`Message sent: ${message}`);
  });

  app.get(`/${username}/spam`, (req, res) => {
    fetch(`http://localhost:${port}/spam`);
    res.send('Spamming');
  });

  app.get(`/${username}/antiafk`, (req, res) => {
    fetch(`http://localhost:${port}/antiafk`);
    res.send('Anti-AFK enabled');
  });

  app.get(`/${username}/health`, (req, res) => {
    fetch(`http://localhost:${port}/health`)
      .then(response => response.text())
      .then(health => {
        res.send(`Bot's current health: ${health}`);
      });
  });

  app.get(`/${username}/players`, (req, res) => {
  ...
  app.get(`/${username}/killaura`, (req, res) => {
    fetch(`http://localhost:${port}/killaura`)
      .then(response => response.text())
      .then(killaura => {
  ...
  app.get(`/${username}/disconnect`, (req, res) => {
    fetch(`http://localhost:${port}/disconnect`);
    res.send('Bot has disconnected');
  });

  app.get(`/${username}/move`, (req, res) => {
    const direction = req.query.direction;
    fetch(`http://localhost:${port}/move?direction=${direction}`);
...
function createBot(username, port){
	...
	bot.on('entitySpawn', (entity) => {
		if (config.securityguard) {
			if (entity.type === 'player' && entity.username != bot.username && !config.blacklist.includes(entity.username)) {
...
function killMyself(){
  ...
  if (lavaBlock) {
    ...
    const p = lavaBlock.position;
      bot.pathfinder.setGoal(new GoalNear(p.x, p.y, p.z, 50));
  } else {
...
wss.on('connection', ws => {
  ...
  ws.on('close', () => {
    // Remove closed connections from the list
      sendWebhookMessage(webhookUrl,"WebSocket disconnected");
...
app.get('/kill', (req, res) => {
    ...
    sendWSmessage('Bot is in the process of suicide');    
...
app.get('/killaura', (req, res) => {
    killaura = !killaura;
    res.send(`Killaura: `+killaura);
    sendWSmessage(`Killaura: `+killaura);
    sendWebhookMessage(webhookUrl, `Killaura: `+killaura);
});
